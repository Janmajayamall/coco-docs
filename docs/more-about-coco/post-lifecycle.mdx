...
Once post is created, it enters the `Prediction Period` followed by `Challenge Period`.
...
This period lasts for some amount of time (can range from hours to days) set by group's moderator.
...
to group's moderator for them to declare the final outcome. The group's moderator
...
period, the moderator is expected to declare the final outcome for the post.

If moderator fails to declare the outcome, the last temporary outcome is set as the
final outcome. Occurance of failure is a signal to users that the moderator isn't
performing their job correctly.

### Resolved

Post's predicition market resolves once final outcome is set, either by group's moderator
